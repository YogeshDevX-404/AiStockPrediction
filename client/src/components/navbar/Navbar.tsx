...
export const Navbar: React.FC<NavbarProps> = ({ onToggleSidebar }) => {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 w-full glass-panel border-b border-white/10 px-4 md:px-6 py-3 transition-all duration-200">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-3">
            {onToggleSidebar && (
              <button
                ...
                className="p-2 rounded-xl text-muted-foreground hover:text-foreground hover:bg-white/10 lg:hidden cursor-pointer"
              >
            ...
            <Link to={ROUTES.DASHBOARD} className="flex items-center space-x-2 group">
              ...
              <div className="hidden sm:block">
                ...
                <span className="ml-1 text-[10px] uppercase font-bold tracking-widest px-1.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                  AI
                </span>
              </div>
            </Link>
            ...
            <div className="hidden xl:flex items-center space-x-2 text-[11px] font-bold">
              <span className="flex items-center px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
          ...
          <div className="flex-1 max-w-md hidden sm:block">
            <button
              ...
              className="w-full glass-panel border border-white/10 rounded-full px-4 py-2 text-xs text-slate-400 flex items-center justify-between hover:border-white/20 transition-all cursor-pointer"
            >
              <div className="flex items-center space-x-2">
                <Search className="w-4 h-4 text-emerald-400" />
                <span>Search stocks, crypto, AI signals...</span>
              </div>
              <kbd className="px-2 py-0.5 rounded-md bg-white/10 text-[10px] font-mono text-slate-300">
                ⌘K
              </kbd>
            </button>
          </div>
          ...
          <div className="flex items-center space-x-2 md:space-x-3">
            ...
            <Dropdown
              trigger={
                <button className="p-2.5 rounded-xl glass-pill hover:bg-white/10 text-muted-foreground transition-colors cursor-pointer">
                  {themeIcons[theme]}
                </button>
              }
            ...
            <button className="relative p-2.5 rounded-xl glass-pill hover:bg-white/10 text-muted-foreground transition-colors cursor-pointer">
              <Bell className="w-4 h-4 text-foreground/80" />
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 bg-emerald-500 text-[10px] font-bold text-white rounded-full flex items-center justify-center shadow-md animate-pulse">
                  {unreadCount}
                </span>
              )}
            </button>
            ...
            <Dropdown
              trigger={
                <div className="flex items-center space-x-2 cursor-pointer p-1 rounded-full hover:bg-white/5 transition-colors">
