    ...
    <div className="space-y-6">
      ...
      <GlassCard className="space-y-4">
        <div className="flex items-center justify-between border-b border-border/50 pb-3">
          <h2 className="text-base font-bold font-display text-foreground">Historical Reliability Benchmarks</h2>
          <Badge variant="emerald">84.8% AVG ACCURACY</Badge>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
          {statistics.map((stat) => (
            <div key={stat.id} className="p-3.5 rounded-2xl bg-foreground/5 border border-border/50 space-y-2">
              <div className="flex items-center justify-between">
                <span className="font-bold text-foreground font-mono">{stat.patternName}</span>
                <span className="text-emerald-600 dark:text-emerald-400 font-extrabold font-mono">{stat.historicalSuccessRate}%</span>
              </div>
              <div className="w-full bg-foreground/10 h-1.5 rounded-full overflow-hidden">
                <div className="bg-emerald-500 dark:bg-emerald-400 h-full rounded-full" style={{ width: `${stat.historicalSuccessRate}%` }} />
              </div>
              <div className="flex justify-between text-[11px] text-muted-foreground border-t border-border/40 pt-1.5">
                <span>Avg Move: <strong className="text-foreground">+{stat.avgMovePercent}%</strong></span>
                <span>Sample: <strong className="text-foreground">{stat.occurrences}</strong></span>
              </div>
            </div>
          ))}
        </div>
      ...
      <GlassCard className="space-y-4">
        <div className="flex items-center justify-between border-b border-border/50 pb-3">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-base font-bold font-display text-foreground">Candlestick Detection Audit Log</h2>
          </div>
            ...
            <tbody className="divide-y divide-white/5">
              {history.map((item) => (
                <tr key={item.id} className="hover:bg-foreground/5">
                  ...
                  <td className="py-3 text-purple-600 dark:text-purple-400 font-bold">{item.confidenceScore}%</td>
                  <td className="py-3 font-sans font-bold">
                    <span className="flex items-center text-emerald-600 dark:text-emerald-400">
                      <CheckCircle2 className="w-3.5 h-3.5 mr-1" /> SUCCESS
                    </span>
                  </td>
                  <td className="py-3 text-emerald-600 dark:text-emerald-400 font-bold">+{item.realizedPnlPercent}%</td>
                  <td className="py-3 text-right text-muted-foreground font-sans">{item.timestamp}</td>
                </tr>
              ))}
            </tbody>
          </table>
