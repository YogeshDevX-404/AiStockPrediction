    ...
    <GlassCard className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/10 pb-3">
        <div className="flex items-center space-x-2">
          <h2 className="text-base font-bold font-display text-white">Smart Watchlist Monitor</h2>
              ...
              return (
                <tr key={item.id} className="hover:bg-white/5 transition-colors">
                  ...
                  >
                    ...
                    <div>
                      <div className="hover:text-emerald-400 transition-colors flex items-center space-x-1">
                        <span>{item.symbol}</span>
                        {item.hasActiveAlert && <Bell className="w-3 h-3 text-amber-400 fill-amber-400" />}
                      </div>
                  ...
                  {/* Price */}
                  <td className="py-3 font-bold text-white font-mono">{formatCurrency(item.price)}</td>
                  ...
                  {/* Confidence */}
                  <td className="py-3 font-extrabold text-purple-400 font-display">{item.confidence}%</td>

                  {/* RSI */}
                  <td className="py-3 font-mono font-semibold text-slate-300">{item.rsi}</td>

                  {/* MACD */}
                  <td className="py-3">
                    <span className={`font-bold ${item.macdStatus === 'BULLISH_CROSS' ? 'text-emerald-400' : 'text-slate-400'}`}>
                  ...
                  <td className="py-3 text-right">
                    <Dropdown
                      ...
                      items={[
                        ...
                        { label: 'Set Price Alert', icon: <Bell className="w-4 h-4 text-purple-400" />, onClick: () => onOpenAlertDrawer(item.symbol) },
                        { label: 'Compare Matrix', icon: <GitCompare className="w-4 h-4 text-amber-400" />, onClick: () => handleCompareAdd(item.symbol) },
                        { label: 'Remove from Watchlist', icon: <Trash2 className="w-4 h-4 text-red-400" />, onClick: () => onDeleteItem(item.id), danger: true },
                      ]}
                    />
                  </td>
                </tr>
