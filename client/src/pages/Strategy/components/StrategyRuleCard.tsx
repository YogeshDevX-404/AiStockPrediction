    ...
    <GlassCard className="p-5 space-y-4 hover:border-border transition-all">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-base font-bold text-foreground font-display">{strategy.name}</h2>
            <p className="text-xs text-muted-foreground font-mono">${strategy.symbol} • Timeframe: {strategy.timeframe}</p>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-border/50 text-xs font-mono">
        <span className="text-emerald-600 dark:text-emerald-400 font-extrabold">+{strategy.netProfitPercent}% Net Profit</span>
        <button
          onClick={() => navigate('/strategy/backtest')}
          className="flex items-center space-x-1 text-purple-600 dark:text-purple-400 hover:text-purple-300 font-sans font-bold cursor-pointer"
        >
