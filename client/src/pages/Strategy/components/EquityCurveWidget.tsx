    ...
    <GlassCard glow className="p-6 space-y-4 border-purple-500/30 bg-gradient-to-br from-purple-950/30 via-card to-emerald-950/20">
      <div className="flex items-center justify-between border-b border-white/10 pb-3">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-base font-bold text-white font-display">Backtest Portfolio Equity Curve</h2>
        </div>
        <span className="text-xs font-mono font-bold text-emerald-400">Initial: $10,000 -&gt; Final: $13,850</span>
      </div>
        ...
        {curve.map((pt) => (
          <div key={pt.timestamp} className="space-y-1 text-xs font-mono">
            <div className="flex justify-between items-center font-bold">
              <span className="text-slate-400 font-sans">{pt.timestamp}</span>
              <span className="text-white">{formatCurrency(pt.equity)} ({pt.drawdownPercent}%)</span>
            </div>
