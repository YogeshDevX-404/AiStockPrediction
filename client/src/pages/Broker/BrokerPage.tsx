    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 glass-panel p-6 border-purple-500/20 bg-gradient-to-r from-purple-950/30 via-card to-emerald-950/20">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-2xl font-black font-display text-foreground">Multi-Broker API Sync & Gateway</h1>
          ...
          <p className="text-xs text-muted-foreground">Connect Zerodha Kite, Upstox, Alpaca, & IBKR accounts with unified portfolio balances and live order dispatching.</p>
        </div>
        ...
        <div className="flex items-center space-x-2 w-full sm:w-auto">
          ...
          <Button variant="glass" size="sm" leftIcon={<Send className="w-4 h-4 text-purple-600 dark:text-purple-400" />} onClick={() => navigate('/broker-sync/terminal')}>
            Order Terminal
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-3">
          <h2 className="text-sm font-bold text-foreground uppercase tracking-wider">Connected Broker Accounts</h2>
          {accounts.map((a) => (
