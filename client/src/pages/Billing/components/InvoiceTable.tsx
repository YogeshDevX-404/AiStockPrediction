...
export const InvoiceTable: React.FC<InvoiceTableProps> = ({ invoices }) => {
  return (
    <GlassCard className="p-5 space-y-4">
      <h2 className="text-base font-bold text-white font-display border-b border-white/10 pb-3">Billing Invoice Statements</h2>
          ...
          <tbody className="divide-y divide-white/5">
            {invoices.map((inv) => (
              <tr key={inv.id} className="hover:bg-white/5">
                ...
                <td className="py-3 text-slate-300">{new Date(inv.createdAt).toLocaleDateString()}</td>
                <td className="py-3 text-white font-bold">{formatCurrency(inv.amount)}</td>
