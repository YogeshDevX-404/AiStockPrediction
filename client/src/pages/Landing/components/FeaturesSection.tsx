...
const features: Feature[] = [
  ...
  },
  {
    ...
    icon: <Camera className="w-6 h-6 text-amber-500 dark:text-amber-400" />,
  ...
  return (
    <section id="features" className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-16 relative">
      <div className="text-center space-y-4 max-w-3xl mx-auto">
        ...
        <h2 className="text-3xl sm:text-5xl font-black font-display tracking-tight text-foreground">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {features.map((feat, idx) => (
          ...
          >
            <GlassCard
              glow
              className="h-full p-6 space-y-4 border border-border/50 hover:border-white/25 transition-all duration-300 group hover:-translate-y-1 bg-[#090e1f]/70"
            >
              <div className="flex items-center justify-between">
                <div className={`p-3 rounded-2xl bg-gradient-to-br ${feat.gradient} border border-border/50 shadow-lg group-hover:scale-110 transition-transform`}>
                  {feat.icon}
                </div>
              ...
              <div className="space-y-2 pt-2">
                <h3 className="text-lg font-bold font-display text-foreground group-hover:text-emerald-600 dark:text-emerald-400 transition-colors">
                  {feat.title}
                </h3>
