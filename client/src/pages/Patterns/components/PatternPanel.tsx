    ...
    >
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h2 className="text-lg font-black text-white font-display">{pattern.patternName}</h2>
              {isPrimary && <Badge variant="purple">PRIMARY PATTERN</Badge>}
              <Badge variant="emerald">{pattern.category}</Badge>
            </div>
            <p className="text-xs text-slate-400">Timeframe: {pattern.timeframe} • Strength: {pattern.strength}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-center">
        <div className="p-3 rounded-2xl bg-purple-500/10 border border-purple-500/30 space-y-0.5">
          ...
          <div className="text-2xl font-black text-purple-400 font-display">{pattern.confidenceScore}%</div>
        ...
        <div className="p-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 space-y-0.5">
          ...
          <div className="text-xl font-black text-emerald-400 font-display">+{pattern.targetMovePercent}%</div>
          <p className="text-[10px] text-emerald-400">Target Upside</p>
        </div>

        <div className="p-3 rounded-2xl bg-white/5 border border-white/10 space-y-0.5 col-span-2 sm:col-span-1">
          <span className="text-[10px] text-slate-400 font-bold uppercase">Signal Reliability</span>
          <div className="text-xl font-extrabold text-white font-mono">HIGH CONVICTION</div>
          <p className="text-[10px] text-slate-400">Volume Confirmed</p>
        </div>
      </div>
