    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 glass-panel p-6 border-purple-500/20 bg-gradient-to-r from-purple-950/30 via-card to-emerald-950/20">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-2xl font-black font-display text-foreground">Operations Console & Control Center</h1>
          ...
          <p className="text-xs text-muted-foreground">Platform infrastructure health, API provider failover, RBAC user security, and AI ops monitoring.</p>
        </div>

        <div className="flex items-center space-x-2 w-full sm:w-auto">
          <Button variant="glass" size="sm" leftIcon={<Users className="w-4 h-4 text-purple-600 dark:text-purple-400" />} onClick={() => navigate('/admin/users')}>
            Manage Users
          </Button>
          <Button variant="glass" size="sm" leftIcon={<Server className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />} onClick={() => navigate('/admin/providers')}>
            API Providers
          </Button>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-bold text-foreground uppercase tracking-wider">Configured API Provider Adapters</h2>
