...
export const PermissionMatrixTable: React.FC = () => {
  ...
  return (
    <GlassCard className="p-5 space-y-4">
      <h2 className="text-base font-bold text-foreground font-display border-b border-border/50 pb-3">RBAC Permission Matrix Grid</h2>
      <div className="overflow-x-auto">
        <table className="w-full text-left text-xs font-mono">
          ...
          <tbody className="divide-y divide-white/5">
            {permissions.map((p) => (
              <tr key={p.name} className="hover:bg-foreground/5">
                <td className="py-3 font-bold text-foreground font-sans">{p.name}</td>
                {p.access.map((hasAccess, idx) => (
                  <td key={idx} className="py-3 text-center">
                    {hasAccess ? (
                      <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400">
                        <Check className="w-3.5 h-3.5" />
                      </span>
                    ) : (
                      <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-slate-500/20 text-muted-foreground">
