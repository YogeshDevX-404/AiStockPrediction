    ...
    <GlassCard glow className="p-6 border-purple-500/40 bg-gradient-to-br from-purple-950/30 via-[#090f24] to-[#050814] space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-emerald-500 via-blue-500 to-purple-600 flex items-center justify-center font-mono font-black text-xl text-white shadow-lg">
            {prediction.symbol.slice(0, 2)}
          </div>
          <div>
            <div className="flex items-center space-x-2">
              <h2 className="text-xl font-extrabold text-white font-display">{prediction.symbol}</h2>
              <Badge variant="purple">{prediction.timeframe} HORIZON</Badge>
              <Badge variant="emerald">{prediction.riskLevel} RISK</Badge>
            </div>
            <p className="text-xs text-slate-400">{prediction.name}</p>
          </div>
        </div>
        ...
        <div className="text-right">
          ...
          <span className="text-[10px] text-slate-400 font-bold uppercase">Signal Conviction</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        >
          ...
          <div className="text-3xl font-black text-purple-400 font-display">{prediction.confidenceScore}%</div>
        ...
        <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 space-y-1">
          ...
          <div className="text-xl font-extrabold text-emerald-400 font-display">{formatCurrency(prediction.targetPrice)}</div>
        ...
        <div className="p-4 rounded-2xl bg-red-500/10 border border-red-500/30 space-y-1">
          ...
          <div className="text-xl font-extrabold text-red-400 font-display">{formatCurrency(prediction.stopLoss)}</div>
          <div className="text-xs text-red-400 font-bold">-6.3% Hazard</div>
        </div>

        {/* Risk-Reward Ratio */}
        <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-1">
          <span className="text-[10px] text-slate-400 uppercase font-bold">Risk-Reward Ratio</span>
          <div className="text-xl font-extrabold text-white font-mono">{prediction.riskRewardRatio} : 1</div>
          <div className="text-[10px] text-slate-400">Entry: <strong className="text-white">${prediction.entryPrice}</strong></div>
        </div>
      </div>

      {/* Rationale Timeline */}
      <div className="space-y-2 text-xs border-t border-white/10 pt-4">
        <span className="font-bold text-white uppercase tracking-wider text-[11px]">Model Driver Rationale:</span>
