    ...
    <GlassCard glow className="p-5 space-y-4 border-purple-500/30 bg-gradient-to-br from-purple-950/30 via-card to-emerald-950/20">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-base font-bold text-foreground font-display">Specialized Agent Entourage</h2>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        {agents.map((ag) => (
          <span key={ag} className="px-3 py-1 rounded-xl bg-foreground/10 border border-border/50 text-xs font-bold text-slate-200">
            🤖 {ag}
          </span>
        ))}
      </div>
