...
export const ChatMessageBubble: React.FC<ChatMessageBubbleProps> = ({ message }) => {
  ...
  return (
    <div className={`flex items-start space-x-3 text-xs ${isUser ? 'flex-row-reverse space-x-reverse' : ''}`}>
      {/* Avatar */}
      <div
        className={`w-9 h-9 rounded-2xl flex items-center justify-center font-bold text-white shrink-0 border ${
          isUser
      ...
      <div
        className={`max-w-2xl rounded-2xl p-4 space-y-3 border ${
          isUser
            ...
            : 'glass-panel border-white/10 bg-card/80 text-slate-200'
        }`}
      >
        {isUser ? (
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between border-b border-white/10 pb-2">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-bold font-display text-white text-sm">{message.summary}</span>
              </div>
            ...
            {/* Detailed Body */}
            <p className="leading-relaxed text-slate-300 whitespace-pre-line">{message.detailedExplanation}</p>
            ...
            {message.confidenceScore && message.confidenceScore > 0 && (
              <div className="p-3 rounded-xl bg-purple-500/10 border border-purple-500/20 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-base font-extrabold text-purple-400 font-display">{message.confidenceScore}%</div>
                </div>
                {message.suggestedNextStep && (
                  <div className="text-right max-w-xs">
                    <span className="text-[10px] text-slate-400 font-bold uppercase">Next Action</span>
                    <p className="text-[11px] text-emerald-400 font-bold">{message.suggestedNextStep}</p>
                  </div>
                )}
              </div>
            )}
            ...
            {/* Actions Bar */}
            <div className="flex items-center justify-between border-t border-white/5 pt-2 text-[10px] text-slate-400 font-mono">
              <span>{new Date(message.timestamp).toLocaleTimeString()}</span>
              <button onClick={handleCopy} className="p-1 hover:text-white cursor-pointer flex items-center space-x-1">
