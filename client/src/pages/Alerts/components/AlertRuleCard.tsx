...
export const AlertRuleCard: React.FC<AlertRuleCardProps> = ({ rule }) => {
  ...
  return (
    <GlassCard className="p-4 space-y-3 hover:border-white/20 transition-all flex items-center justify-between">
      <div className="flex items-center space-x-3">
        ...
        <div>
          <div className="flex items-center space-x-2">
            <h3 className="text-sm font-bold text-white font-display">{rule.triggerCondition}</h3>
            <Badge variant={rule.priority === 'URGENT' ? 'red' : 'purple'}>{rule.priority}</Badge>
          </div>
          <span className="text-[11px] text-slate-400 font-mono">Triggered {rule.triggerCount} times • Created {rule.createdAt}</span>
        </div>
      </div>
