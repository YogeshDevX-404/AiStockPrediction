    ...
    <GlassCard glow className="p-6 border-purple-500/40 bg-gradient-to-br from-purple-950/30 via-[#0a0e24] to-[#060914] space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
        <div className="flex items-center space-x-3">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-purple-500 to-emerald-500 flex items-center justify-center font-mono font-black text-xl text-white shadow-lg">
            {currentAnalysis.ticker.slice(0, 2)}
          </div>
          <div>
            <div className="flex items-center space-x-2">
              <h2 className="text-xl font-extrabold text-white font-display">{currentAnalysis.ticker} OCR DETECTED</h2>
              <Badge variant="purple">{currentAnalysis.timeframe} CHART</Badge>
              <Badge variant="emerald">{currentAnalysis.chartType}</Badge>
            </div>
            <p className="text-xs text-slate-400">Scanned Layout: TradingView / Kite • Model: GPT-4 Vision Neural Pipeline</p>
          </div>
        </div>
        ...
        <div className="text-right">
          ...
          <span className="text-[10px] text-slate-400 font-bold uppercase">Vision Probability Rating</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
        <div className="p-4 rounded-2xl bg-purple-500/10 border border-purple-500/30 space-y-1">
          ...
          <div className="text-3xl font-black text-purple-400 font-display">{currentAnalysis.confidenceScore}%</div>
        ...
        <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 space-y-1">
          ...
          <div className="text-xl font-extrabold text-emerald-400 font-display">{formatCurrency(currentAnalysis.targetPrice)}</div>
          <div className="text-xs text-emerald-400 font-bold">Projected Resistance</div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-red-500/10 border border-red-500/30 space-y-1">
          ...
          <div className="text-xl font-extrabold text-red-400 font-display">{formatCurrency(currentAnalysis.stopLoss)}</div>
          <div className="text-xs text-red-400 font-bold">Key Support Breach</div>
        </div>

        <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-1">
          <span className="text-[10px] text-slate-400 uppercase font-bold">Suggested Entry Zone</span>
          <div className="text-base font-extrabold text-white font-mono">{currentAnalysis.entryZone}</div>
          <div className="text-[10px] text-slate-400">Risk Profile: <strong className="text-emerald-400">{currentAnalysis.riskLevel}</strong></div>
        </div>
      </div>
      ...
      <div className="space-y-2 text-xs border-t border-white/10 pt-4">
        <span className="font-bold text-white uppercase tracking-wider text-[11px]">Neural Vision Driver Rationale:</span>
