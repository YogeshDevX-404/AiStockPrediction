  ...
  return (
    <GlassCard className="space-y-4">
      <div className="flex items-center space-x-2 border-b border-border/50 pb-3">
        ...
        <h2 className="text-base font-bold font-display text-foreground">Sector Allocation</h2>
      </div>

      <div className="space-y-3 text-xs">
        <div className="w-full h-3 rounded-full overflow-hidden flex space-x-0.5 bg-foreground/10 p-0.5">
          {sectors.map((s) => (
            <div
              key={s.sector}
        ...
        <div className="space-y-2 pt-1">
          {sectors.map((s) => (
            <div key={s.sector} className="flex justify-between items-center text-muted-foreground">
              <div className="flex items-center space-x-2">
                <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: s.color }} />
                <span className="truncate">{s.sector}</span>
              </div>
              <span className="font-bold text-foreground font-mono">{s.percentage}%</span>
            </div>
          ))}
        </div>
      </div>
